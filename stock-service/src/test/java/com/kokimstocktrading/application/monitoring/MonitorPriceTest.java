...
public class MonitorPriceTest {
  ...
  /**
   * 종목 코드 : "005930", 시작 가격 : "75000", 종목 이름 : 삼성전자 종목 코드 : "000660", 시작 가격 : "85000", 종목 이름 :
   * SK하이닉스 종목 코드 : "035420", 시작 가격 : "850000", 종목 이름 : NAVER 종목 코드 : "051910", 시작 가격 : "4500" , 종목
   * 이름 : LG화학 종목 코드 : "006400", 시작 가격 : "35000", 종목 이름 : 삼성SDI 다음 시세의 변동폭은 전 시세의 +- 5 % 입니다.
   */
  ...
  public void canRemoveConditionById() {
    //given
    PriceCondition condition1 = new PriceCondition(UUID.randomUUID(), "005930", 76000L, () -> {
    }, TouchDirection.FROM_BELOW);
    PriceCondition condition2 = new PriceCondition(UUID.randomUUID(), "005930", 77000L, () -> {
    }, TouchDirection.FROM_BELOW);
  ...
  public void conditionRemovedAfterAchievement() {
    //given
    AtomicBoolean triggered = new AtomicBoolean(false);
    PriceCondition condition = new PriceCondition(UUID.randomUUID(), "005930", 74000L,
        // 시작가보다 낮은 조건 (즉시 달성)
  ...
  public void canGetAllConditions() {
    ...
    PriceCondition condition2 = new PriceCondition(UUID.randomUUID(), "000660", 86000L, () -> {
    }, TouchDirection.FROM_BELOW);
    PriceCondition condition3 = new PriceCondition(UUID.randomUUID(), "005930", 77000L, () -> {
    }, TouchDirection.FROM_BELOW);
  ...
  public void canReceiveRealTimeQuoteFromMockAdapter() {
    ...
    //when
    Flux<RealTimeQuote> quoteFlux = subscribeRealTimeQuotePort.subscribeStockQuote(
        List.of(stockCode));
