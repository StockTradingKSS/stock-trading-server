...
public class TradingConditionPersistenceAdapter implements SaveTradingConditionPort {
    ...
    /**
     * 이평선 조건 조회 (ID)
     * NOTE: callback은 빈 Runnable로 반환됨 (DB에 저장되지 않음)
     */
    @Override
    ...
    public Optional<MovingAverageCondition> findMovingAverageConditionById(UUID conditionId) {
        ...
        try {
            Optional<MovingAverageCondition> result = movingAverageConditionRepository.findById(conditionId)
    ...
    /**
     * 모든 활성화된 이평선 조건 조회
     * NOTE: callback은 빈 Runnable로 반환됨
     */
    @Override
    @Transactional(readOnly = true)
