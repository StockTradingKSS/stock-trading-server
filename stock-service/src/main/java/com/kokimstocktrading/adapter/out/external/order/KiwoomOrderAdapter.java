...
public class KiwoomOrderAdapter implements RequestStockOrderPort {
  ...
  @Override
  public Mono<OrderResult> requestBuyStock(String stockCode, int quantity, double price,
      TradeType tradeType) {
    ...
    return kiwoomAuthAdapter.getValidToken()
        ...
        .flatMap(response -> {
          ...
          log.info("매수 주문 성공: orderNo={}, message={}", response.orderNo(),
  ...
  @Override
  public Mono<OrderResult> requestSellStock(String stockCode, int quantity, double price,
      TradeType tradeType) {
    // OrderType을 String 코드로 변환
    String typeCode = tradeType != null ? tradeType.getCode() : TradeType.LIMIT.getCode();
    ...
    return kiwoomAuthAdapter.getValidToken()
        ...
        .flatMap(response -> {
          ...
          log.info("매도 주문 성공: orderNo={}, message={}", response.orderNo(),
              response.returnMessage());
          return Mono.just(
  ...
  @Override
  public Mono<OrderResult> requestModifyOrder(String orderNo, String stockCode, int quantity,
      double price) {
  ...
  public Mono<OrderResult> requestCancelOrder(String orderNo, String stockCode, int quantity) {
    CancelOrderRequest request = CancelOrderRequest.createPartialCancel(orderNo, stockCode,
        quantity);

    return executeCancelOrder(request, stockCode, quantity);
  }

  private Mono<OrderResult> executeCancelOrder(CancelOrderRequest request, String stockCode,
      int quantity) {
  ...
  public record ModifyOrderRequest(
    ...
    public static ModifyOrderRequest of(String orderNo, String stockCode, int quantity,
        double price) {
  ...
  public record OrderRequest(
    ...
    public static OrderRequest createBuyOrder(String stockCode, int quantity, double price,
        String tradeType) {
    ...
    public static OrderRequest createSellOrder(String stockCode, int quantity, double price,
        String tradeType) {
      // 시장가 주문인 경우(tradeType="3") 단가 필드 null로 처리
      String priceStr = "3".equals(tradeType) ? null : String.valueOf(Math.round(price));
  ...
  private record CancelOrderRequest(
    ...
    public static CancelOrderRequest createPartialCancel(String orderNo, String stockCode,
        int quantity) {
      return new CancelOrderRequest(
          "SOR",
          orderNo,
