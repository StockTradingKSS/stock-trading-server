...
@Slf4j
public class MinuteStockCandleBulkRepositoryImpl implements StockCandleBulkRepository<MinuteStockCandleEntity> {
    ...
    @Override
    public Mono<Void> bulkInsert(List<MinuteStockCandleEntity> entities) {
        if (entities.isEmpty()) {
            return Mono.empty();
        }

        log.info("MinuteCandle 병렬 Bulk insert 시작: {}개 엔티티, 배치크기: {}, 동시성: {}", 
        ...
        return Flux.fromIterable(entities)
                ...
                .doOnSuccess(v -> {
                    ...
                    log.info("MinuteCandle 병렬 Bulk insert 완료: {}개 엔티티, 총 소요시간: {}ms", 
                            entities.size(), endTime - startTime);
                });
    }

    private Mono<Void> insertBatch(List<MinuteStockCandleEntity> batch) {
        if (batch.isEmpty()) {
            return Mono.empty();
        }
        ...
        StringBuilder sql = new StringBuilder("""
            INSERT INTO minute_stock_candle 
        ...
        // 명명된 파라미터 바인딩
        for (int i = 0; i < batch.size(); i++) {
            MinuteStockCandleEntity entity = batch.get(i);
        ...
        return spec.then()
                ...
                .doOnSuccess(v -> {
                    long endTime = System.currentTimeMillis();
                    log.debug("MinuteCandle 배치 삽입 완료: {}개, 소요시간: {}ms, 스레드: {}", 
                            batch.size(), endTime - startTime, Thread.currentThread().getName());
                })
                .onErrorResume(e -> {
                    log.error("MinuteCandle Bulk insert 실패 (배치크기: {}), 개별 저장으로 폴백: {}", batch.size(), e.getMessage());
     ...
     */
    private Mono<Void> fallbackToParallelIndividualInsert(List<MinuteStockCandleEntity> batch) {
        return Flux.fromIterable(batch)
                .flatMap(this::insertSingle, 20) // 개별 저장도 병렬 처리
                .then()
                .doOnSuccess(v -> log.info("MinuteCandle 병렬 개별 저장 완료: {}개", batch.size()));
    }

    private Mono<Void> insertSingle(MinuteStockCandleEntity entity) {
        String sql = """
            INSERT INTO minute_stock_candle 
            (code, current_price, previous_price, volume, open_price, high_price, low_price, close_price, open_time) 
    ...
    @Override
    public Flux<MinuteStockCandleEntity> bulkSaveAll(Flux<MinuteStockCandleEntity> entities) {
