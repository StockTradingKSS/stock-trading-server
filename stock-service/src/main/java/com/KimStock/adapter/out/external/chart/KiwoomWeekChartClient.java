...
public class KiwoomWeekChartClient {
    ...
    public Mono<List<StockCandle>> loadWeekCandles(WeekStockCandleRequest request) {
        return kiwoomAuthAdapter.getValidToken()
                .flatMap(token -> executeLoadWeekStockCandleListApiBy(token, request))
                .flatMap(response -> {
    ...
    private Mono<WeekStockCandleResponse> executeLoadWeekStockCandleListApiBy(String token, WeekStockCandleRequest request) {
        String nextKey = "";
        return webClient.post()
                ...
                .bodyValue(request)
                .exchangeToMono(this::handleWeekChartResponse);
    }

    private Mono<WeekStockCandleResponse> handleWeekChartResponse(ClientResponse response) {
        if (response.statusCode().is2xxSuccessful()) {
            ...
            return response.bodyToMono(WeekStockCandleResponse.class)
                    .doOnSuccess(orderResponse -> {
                        if (orderResponse.isSuccess()) {
                            log.info("Week Stock Candle API call successful size: {}", orderResponse.weekCandleItems().size());
                        } else {
                            log.warn("Week Stock Candle returned error: {}", orderResponse.returnMsg());
                        }
                    });
        }
        return clientErrorHandler.handleErrorResponse(response, "Week Stock Candle API call failed. ");
    }

    public record WeekStockCandleRequest(
            String stk_cd, // 종목 코드
            String base_dt, // 기준일자 YYYYMMDD
            String upd_stkpc_tp // 수정주가구분
    ) {
        @Builder
        public WeekStockCandleRequest {
            stk_cd = Objects.requireNonNullElse(stk_cd, "039490");
        }

        public static WeekStockCandleRequest of(String stockCode, boolean isUpdatedPrice, LocalDateTime lastDateTime) {
            return WeekStockCandleRequest.builder()
     ...
     * 주식 차트 응답 DTO
     */
    public record WeekStockCandleResponse(
            @JsonProperty("stk_cd") String stockCode,
            @JsonProperty("stk_stk_pole_chart_qry") List<WeekCandleItem> weekCandleItems,
            @JsonProperty("return_code") Integer returnCode,
            @JsonProperty("return_msg") String returnMsg
    ) {
        @Builder
        public WeekStockCandleResponse {
            stockCode = Objects.requireNonNullElse(stockCode, "");
            weekCandleItems = Objects.requireNonNullElse(weekCandleItems, new ArrayList<>());
