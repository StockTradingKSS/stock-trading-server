...
@Component
@Slf4j
public class KiwoomMonthChartClient {
    ...
    public Mono<List<StockCandle>> loadMonthCandles(MonthStockCandleRequest request) {
        return kiwoomAuthAdapter.getValidToken()
                .flatMap(token -> executeLoadMonthStockCandleListApiBy(token, request))
                .flatMap(response -> {
                    ...
                    log.info("차트 조회 성공");
                    List<MonthStockCandleResponse.MonthCandleItem> MonthCandleItemList = response.MonthCandleItems();
                    List<StockCandle> stockCandleList = new ArrayList<>(MonthCandleItemList.stream()
                            .map(MonthCandleItem -> MonthCandleItem.mapToStockCandle(getOriginalStockCode(request.stk_cd())))
                            .toList());
                ...
                .onErrorResume(e -> {
                    log.error("[Load Month Candle Error] : {}", e.getMessage());
                    return Mono.error(e);
                });
    }

    private Mono<MonthStockCandleResponse> executeLoadMonthStockCandleListApiBy(String token, MonthStockCandleRequest request) {
        String nextKey = "";
        return webClient.post()
                ...
                .bodyValue(request)
                .exchangeToMono(this::handleMonthChartResponse);
    }

    private Mono<MonthStockCandleResponse> handleMonthChartResponse(ClientResponse response) {
        if (response.statusCode().is2xxSuccessful()) {
            ...
            return response.bodyToMono(MonthStockCandleResponse.class)
                    .doOnSuccess(orderResponse -> {
                        if (orderResponse.isSuccess()) {
                            log.info("Month Stock Candle API call successful size: {}", orderResponse.MonthCandleItems().size());
                        } else {
                            log.warn("Month Stock Candle returned error: {}", orderResponse.returnMsg());
                        }
                    });
        }
        return clientErrorHandler.handleErrorResponse(response, "Month Stock Candle API call failed. ");
    }

    public record MonthStockCandleRequest(
            String stk_cd, // 종목 코드
            String base_dt, // 기준일자 YYYYMMDD
            String upd_stkpc_tp // 수정주가구분
    ) {
        @Builder
        public MonthStockCandleRequest {
     ...
     * 주식 차트 응답 DTO
     */
    public record MonthStockCandleResponse(
            @JsonProperty("stk_cd") String stockCode,
            @JsonProperty("stk_mth_pole_chart_qry") List<MonthCandleItem> MonthCandleItems,
            @JsonProperty("return_code") Integer returnCode,
            @JsonProperty("return_msg") String returnMsg
    ) {
        @Builder
        public MonthStockCandleResponse {
            stockCode = Objects.requireNonNullElse(stockCode, "");
            MonthCandleItems = Objects.requireNonNullElse(MonthCandleItems, new ArrayList<>());
